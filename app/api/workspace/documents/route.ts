...
export async function GET(request: Request) {
  ...
  if (!isDjangoAuthEnabled()) {
    return NextResponse.json({ error: "أرشيف المستندات يتطلب ربط API." }, { status: 503 });
  }
  ...
  const { status, data } = await djangoJson<{ documents?: unknown[] }>(
    `/api/workspace/documents/${qs}`,
    { accessToken: access ?? undefined }
  );
  if (status !== 200) {
    return NextResponse.json(
      { error: djangoErrorMessage(data, "تعذر تحميل الأرشيف.") },
      { status }
    );
  }
